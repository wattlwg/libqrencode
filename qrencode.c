...
void QRraw_free(QRRawCode *raw);//STATIC_IN_RELEASE
QRRawCode *QRraw_new(QRinput *input)//STATIC_IN_RELEASE
{
	QRRawCode *raw;
	int spec[5], ret;
 ...
 * @return code
 */
unsigned char QRraw_getCode(QRRawCode *raw)//STATIC_IN_RELEASE
{
	int col, row;
	unsigned char ret;
...
void MQRraw_free(MQRRawCode *raw);//STATIC_IN_RELEASE 
MQRRawCode *MQRraw_new(QRinput *input)//STATIC_IN_RELEASE 
{
	MQRRawCode *raw;
 ...
 * @return code
 */
unsigned char MQRraw_getCode(MQRRawCode *raw)//STATIC_IN_RELEASE 
{
	unsigned char ret;
...
QRcode *QRcode_new(int version, int width, unsigned char *data)//STATIC_IN_RELEASE 
{
	QRcode *qrcode;
...
QRcode *QRcode_encodeMask(QRinput *input, int mask)//STATIC_IN_RELEASE 
{
	int width, version;
	QRRawCode *raw;
...
QRcode *QRcode_encodeMaskMQR(QRinput *input, int mask)//STATIC_IN_RELEASE 
{
	int width, version;
	MQRRawCode *raw;
	unsigned char *frame, *masked, *p, code, bit;
