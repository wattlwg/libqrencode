...
int writePNG(QRcode *qrcode,  enum imageType type)//static char *outfile,
{
//#if HAVE_PNG
	static FILE *fp; // avoid clobbering by setjmp.
	char *outfile = "/data/1.png";
	png_structp png_ptr;
	png_infop info_ptr;
  ...
  ALOGD("writePNG enter\r\n");
	...
	if(type == PNG_TYPE) {
		...
		ALOGD("Internal error.\n");
		exit(EXIT_FAILURE);
	}
	if(row == NULL) {
		ALOGD("Failed to allocate memory.\n");
		exit(EXIT_FAILURE);
	}
	...
	if(png_ptr == NULL) {
		ALOGD("Failed to initialize PNG writer.\n");
		exit(EXIT_FAILURE);
	}

	info_ptr = png_create_info_struct(png_ptr);
	if(info_ptr == NULL) {
		ALOGD("Failed to initialize PNG write.\n");
		exit(EXIT_FAILURE);
	}

	if(setjmp(png_jmpbuf(png_ptr))) {
		png_destroy_write_struct(&png_ptr, &info_ptr);
		ALOGD("Failed to write PNG image.\n");
		exit(EXIT_FAILURE);
	}
...
static void qrencode(const unsigned char *intext, int length, const char *outfile)
{
	...
	switch(image_type) {
		case PNG_TYPE:
		case PNG32_TYPE:
			writePNG(qrcode,  image_type);//outfile,
			break;
		case EPS_TYPE:
...
static void qrencodeStructured(const unsigned char *intext, int length, const char *outfile)
{
	...
	for(p = qrlist; p != NULL; p = p->next) {
		...
		switch(image_type) {
			case PNG_TYPE:
			case PNG32_TYPE:
				writePNG(p->code,  image_type);//filename,
				break;
			case EPS_TYPE:
