...
void MMask_writeFormatInformation(int version, int width, unsigned char *frame, int mask, QRecLevel level)//STATIC_IN_RELEASE 
{
	unsigned int format;
	unsigned char v;
	int i;
...
int MMask_evaluateSymbol(int width, unsigned char *frame)//STATIC_IN_RELEASE 
{
	int x, y;
	unsigned char *p;
	int sum1 = 0, sum2 = 0;
